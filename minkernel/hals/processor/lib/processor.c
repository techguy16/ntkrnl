...
--*/
{
    ...
    //
    // 4chan: server2003 change, adds check for DeviceExtension->PerfStates->Count > 1
    // can't seem to update above while keeping it matching though, strange.
    //
    bufSize = sizeof(PROCESSOR_STATE_HANDLER2);
    if (DeviceExtension->PerfStates) {
        if (DeviceExtension->PerfStates->Count > 1) {
            bufSize += sizeof(PROCESSOR_PERF_LEVEL) * (DeviceExtension->PerfStates->Count - 1);
        }
    }
