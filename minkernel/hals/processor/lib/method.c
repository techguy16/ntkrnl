...
Revision History:
    10/05/20 - ported from XP source tree to server2003
    10/06/20 - backported changes from server2003's compiled processr.sys into this code
               (search '4chan' to find backported changes)
...
--*/
{
    ...
    *AcpiMulti = (PACPI_BIOS_MULTI_NODE) ExAllocatePoolWithTag(NonPagedPool,
    ...
    //
    // 4chan: server2003 change, removed try/except & added valueInfo->DataLength check
    //
    //try {

        int15Info = (PLEGACY_GEYSERVILLE_INT15)&(multiNode->E820Entry[multiNode->Count]);
        if (((ULONG)int15Info - (ULONG)valueInfo) <= valueInfo->DataLength && 
            int15Info->Signature == 'GS') {
